...
def part1(input):
    rgx1 = regex.compile(r'(?<=\[)(.*?)(?=\])')
    rgx2 = regex.compile(r'(?<=\])(.*?)(?=\[)')
    abba = regex.compile(r'(.)(.)\2\1')
    answer = 0
    for l in input:
        inside = regex.findall(rgx1, l)
        outside = [l[:l.index('[')]] + regex.findall(rgx2, l) + [l[::-1][:l[::-1].index(']')][::-1]]
        i, o = 0, 0
        for q in inside:
            m = regex.findall(abba, q)
            for j in m:
                if (j[0] != j[1]):
                    i += 1
                    
        for p in outside:
            m = regex.findall(abba, p)
            for d in m:
                if (d[0] != d[1]):
                    o += 1
...
def part2(input):
    rgx1 = regex.compile(r'(?<=\[)(.*?)(?=\])')
    rgx2 = regex.compile(r'(?<=\])(.*?)(?=\[)')
    aba = regex.compile(r'([a-z]{1})([a-z]{1})(\1)')
    answer = 0
    o, i = 0, 0
    ...
    for l in input:
        ...
        for z in inside:
            g = regex.findall(aba, z, overlapped=True)
            for y in g:
                w = "".join(y)
                if (w[0] != w[1]):
                    ii.append(w)
        ...
        for h in outside:
            k = regex.findall(aba, h, overlapped=True)
            for c in k:
                s = "".join(c)
                if (s[0] != s[1]):
                    oo.append(s)
