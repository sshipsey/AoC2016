...
giveOne = re.compile(r'value (\d+) goes to bot (\d+)')
giveTwo = re.compile(r'bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)')
...
def createBots(input):
    bots = {}
    for line in input:
        if (line.startswith('v')):
            match = re.search(giveOne, line).groups()
            try:
                bots[match[1]].update({'val': [bots[match[1]]['val'][0],int(match[0])]})
            except KeyError:
                bots[match[1]] = {'val': [int(match[0])]}

    for line in input:
        if (line.startswith('b')):
            match = re.search(giveTwo, line).groups()
            try:
                bots[match[0]].update({'hi': int(match[4]), 'lo': int(match[2]), 'hiOut': out(match[3]), 'loOut': out(match[1])})
            except KeyError:
                bots[match[0]] = {'hi': int(match[4]), 'lo': int(match[2]), 'hiOut': out(match[3]), 'loOut': out(match[1])}
                
    for bot in bots:
        if not 'val' in bots[bot]:
            bots[bot]['val'] = []
            
    return bots

def resolve(bots, outputs=[0] * 1000):
    thisBot, a = {'val': None }, 0
    for bot in bots:
            
        if ( len( bots[bot]['val'] ) > 1 ):               
            givingBot = bots[bot]
            thisBot = copy.deepcopy(givingBot)
            a = bot
            hi = str(givingBot['hi'])
            lo = str(givingBot['lo'])

            if (givingBot['hiOut']):
                outputs[int(hi)] = max(givingBot['val'])
            
            else:
                bots[hi]['val'].append(max(givingBot['val']))
            givingBot['val'].remove(max(givingBot['val']))
                    
            if (givingBot['loOut']):
                outputs[int(lo)] = givingBot['val'][0]
            
            else:
                bots[lo]['val'].append(givingBot['val'][0])
            givingBot['val'].remove(givingBot['val'][0])
            
            break
            
    return thisBot, a, outputs
    
def part1(bots):
       
    thisBot, a, outputs = { 'val': None }, 0, [0] * 1000
    
    while(thisBot['val'] != [61, 17] and thisBot['val'] != [17, 61]):
        
        thisBot, a, outputs = resolve(bots)
...
def part2(bots):
    ...
    thisBot, outputs = { 'val': None }, [0] * 1000
    
    while (outputs[0] == 0 or outputs[1] == 0 or outputs[2] == 0):
        
        thisBot, a, outputs = resolve(bots,outputs)

    return outputs[0] * outputs[1] * outputs[2]
   

def day10(input):
    originalBots1 = createBots(input)
    originalBots2 = createBots(input)
    return(part1(originalBots1), part2(originalBots2))
